...
import { Clock, BookOpen, Award, Lock, Crown, PlayCircle } from "lucide-react";
...
const TestManagementCourse = () => {
  ...
  let totalLessons = 0;
  allTestManagementCourses.forEach(course => {
      ...
      <div className="pt-32 pb-20 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-16 animate-fade-in">
            <Badge className="mb-6 bg-gradient-button">TMAP Next • {totalLessons}+ Lessons</Badge>
            <h1 className="text-5xl md:text-6xl font-bold mb-6 bg-gradient-text bg-clip-text text-transparent">
              Test Management Mastery
            </h1>
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-16">
            <Card className="bg-[hsl(48,96%,75%)] border-none hover:scale-[1.02] transition-all duration-300">
              <CardHeader>
                ...
                <CardDescription className="text-foreground/70">TMAP fundamentals, test processes, documentation</CardDescription>
              </CardHeader>
            </Card>
            <Card className="bg-[hsl(145,70%,65%)] border-none hover:scale-[1.02] transition-all duration-300">
              <CardHeader>
                ...
                <CardDescription className="text-foreground/70">Risk-based testing, metrics, stakeholder management</CardDescription>
              </CardHeader>
            </Card>
            <Card className="bg-[hsl(195,90%,65%)] border-none hover:scale-[1.02] transition-all duration-300">
              <CardHeader>
                ...
                <CardDescription className="text-foreground/70">Director-level strategy, governance, org leadership</CardDescription>
              </CardHeader>
            </Card>
          </div>
          ...
          <div className="space-y-12">
            {allTestManagementCourses.map((course, courseIndex) => {
              let lessonNumber = 0;
              for (let i = 0; i < courseIndex; i++) {
                allTestManagementCourses[i].sections.forEach(section => {
                        ...
                        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
                          {section.lessons.map((lesson) => {
                            ...
                            return (
                              <Link
                                key={lesson.id}
                                to={hasAccess ? `/test-management-lesson/${lesson.id}` : '#'}
                                onClick={handleClick}
